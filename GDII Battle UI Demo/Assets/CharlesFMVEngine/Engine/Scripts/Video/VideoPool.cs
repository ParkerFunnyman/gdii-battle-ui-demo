...
namespace CharlesEngine
{
	public class VideoPool : MonoBehaviour
	{
		...
		private List<CEVideoPlayer> _players = new();
...
#if ENABLE_INPUT_SYSTEM
		Vector2 direction = Mouse.current != null ? Mouse.current.delta.ReadValue() : Vector2.zero; // TODO WRONG
#elif ENABLE_LEGACY_INPUT_MANAGER
        Vector2 direction = new Vector2(Input.GetAxis("Mouse X"), Input.GetAxis("Mouse Y")); //TODO 
#else
		...
		private CEVideoPlayer SpawnPlayer()
		{
		...
		*/
		public bool PrepareVideo(VideoClip clip)
		{
			...
			CEVideoPlayer playerNext = SpawnPlayer();
